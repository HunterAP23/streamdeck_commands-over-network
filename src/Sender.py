...
# Class for sending commands to receivers over the connection.
...
# Class for creating Command objects, which are stored in a database
class Command:
...
class Command_Sender:
    def __init__(self):
        self.senderSocket =
        self.receiverSockets = dict()

    def add_receiver(self, node_name, rec_addr):
        try:
            socket.inet_aton(rec_addr)

            receiverSockets[node_name] = rec_addr
        except socket.error:
            return None

    def send_command(self, command, receivers):
        return None
